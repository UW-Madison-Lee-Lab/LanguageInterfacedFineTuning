...
def add_noise(X, sigma):
    ...
    X_noisy = X + torch.randn_like(X) * sigma * 5 
...
def get_adv_ex(data='mnist', pretrained=False, eps=0.3, test_mlp_attack=0):
    ...
    with open(f'mnist_y_train.npy', 'wb') as f:
        np.save(f, y_train)
    with open(f'mnist_adv_X_test{eps_str}.npy', 'wb') as f:
        np.save(f, X_adv_test)
    with open(f'mnist_adv_y_test{eps_str}.npy', 'wb') as f:
        np.save(f, y_test)
    ...
    if test_mlp_attack:
        ...
        # train model // test on clean data & adversarial data
        model = train(model, train_loader, name='mlp')
        test(model, test_loader)
