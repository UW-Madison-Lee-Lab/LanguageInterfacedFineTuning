...
if is_adv:
    if npy_stored:
        ...
        X, y, X_test, y_test = get_adv_ex(data='mnist', eps=args.eps, test_mlp_attack=args.mlp_transfer)
else:
    X, y, X_test, y_test = mnist.load(data_name)
#pdb.set_trace()
if is_noisy:
    X_test = add_noise(X_test, args.sigma)
...
if is_adv:
    np.save(f'data/{permuted}{data_name}_adv{eps_str}', data)
elif is_noisy:
    np.save(f'data/{permuted}{data_name}_noisy_{sigma_str}', data)
else:
...
if is_adv:
    ...
    test_js = write_jsonl('\n'.join(test_prompts), f'{permuted}{data_name}_adv_{eps_str}_test.jsonl')
elif is_noisy:
    #train_js = write_jsonl('\n'.join(train_prompts), f'{permuted}{data_name}_train.jsonl')
    #val_js = write_jsonl('\n'.join(val_prompts), f'{permuted}{data_name}_val.jsonl')
    test_js = write_jsonl('\n'.join(test_prompts), f'{permuted}{data_name}_noisy_{sigma_str}_test.jsonl')
else:
    train_js = write_jsonl('\n'.join(train_prompts), f'{permuted}{data_name}_train.jsonl')
    val_js = write_jsonl('\n'.join(val_prompts), f'{permuted}{data_name}_val.jsonl')
