...
class GPTJFineTuner(object):
    ...
    def eval(self,valid_prompts,valid_df,test_prompts,test_df,logf,y_name='y',train_df = None,imbalance=False):
        """
        ...
        for model_idx in range(len(self.config['epochs'])):
            ...
            if imbalance:
                ...
                acc_val = round(accuracy_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
                f1_val = round(f1_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
                
                precision = round(precision_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
                recall = round(recall_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
                acc = round(accuracy_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
                f1 = round(f1_score(valid_valid_y, valid_valid_y_outputs) * 100, 2)
