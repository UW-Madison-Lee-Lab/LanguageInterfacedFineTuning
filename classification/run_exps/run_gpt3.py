...
if args.task == cfgs.ACCURACY:
    ...
    for data_id in data_ids:
        for run_idx in range(start_idx,end_idx):
            csv_fpath = f"results/csv/run_gpt3_{args.task}_featname_{args.use_feature_name}_incon_{args.in_context}_subset_{args.subset}_fraction_{args.subset_fraction}_did_{args.data_id}_runidx_{args.run_idx}.csv"
            # load dataset
    ...
    for data_id in data_ids:
        for run_idx in range(start_idx,end_idx):
            ...
            if len(y_train) + len(y_val) + len(y_test) > 2*(sum(y_train)+sum(y_val)+sum(y_test)):
                ...
                print(type(y_val),type(y_pred_val),type(y_test),type(y_pred))
                y_val = (-1*(y_val-1)).astype("int")
                y_pred_val = (-1*(np.array(y_pred_val)-1)).astype("int")
                y_test = (-1*(y_test-1)).astype("int")
                y_pred = (-1*(np.array(y_pred)-1)).astype("int")
            ...
            try:
                pd.DataFrame(y_pred.tolist()).to_csv(csv_fpath)
            except:
                print("!!!! can't save y_pred")
