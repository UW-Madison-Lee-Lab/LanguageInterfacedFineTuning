...
def collect_data_json(file_list, metric = 'RAE', grid_loss = False, gpt3 = True, gptj = True, baseline = True):
    """
...
def print_overleaf_basic(folder_name, num_sims = 3, manipulate = 'noise level', p = 1, metric = 'RAE', metrics = metrics):
    mean_df = pd.read_csv(os.path.join(folder_name,'syn_mean_across_%dsims.csv' % num_sims))
    std_df = pd.read_csv(os.path.join(folder_name, 'syn_std_across_%dsims.csv' % num_sims))
        ...
        for integer in [True, False]:
            for positive in [True, False]:
                if integer and positive:
                    ...
                    rg = '(-10, 10)'
                report_idx += (mean_df.noise_level == 0.1) & (mean_df['integer'] == integer) & (mean_df['(lb,ub)'] == rg) & (mean_df.p == p) & ((mean_df['function'] == 'cosine') | (mean_df['function'] == 'exponential'))
...
Method & \gptt{} & \gptj{} & \qr{} & \krr{} & \knn{}  & \ann{} & \xg{} & rf{} \\
  & $\gX$ & %s  & %s  & %s  &  %s &  %s  &  %s &  %s & %s \\ \midrule
        ...
        for i in range(8):
            ...
            if first_func_idx:
                row = '\multirow{4}{*}{%s} & $%s$' % (rows[func_idx], region(mean_sum.loc[i]['(lb,ub)']))
            else:
                row = '& $%s$' %  region(mean_sum.loc[i]['(lb,ub)'])
